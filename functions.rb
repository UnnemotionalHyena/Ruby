...
def extract_accounts_data(browser)
    ...
    account_data.each do |div|
        ...
        account_list.push(Account.new)

        account_list[i].set_name(string[1])
        account_list[i].set_currency(string[0])
        account_list[i].set_balance(string[3])
        account_list[i].set_nature('Account')
...
def exctract_card_data(browser, nr_accounts, account_list)
    ...
    card_name.each do |div|
        ...
        account_list[nr_accounts].set_name(card_data[1])
        account_list[nr_accounts].set_currency(card_data[3])
        account_list[nr_accounts].set_balance(card_data[2])
        account_list[nr_accounts].set_nature('Card')
...
def extract_transactions (browser, account_data, nr_months)
    ...
    account_data.each do |account|
        ...
        if bool_tr == false
            ...
            transactions.push(Transactions.new)
            ...
            browser.div(class: 'operations').wait_until_present.divs.each do |div|
                if div.class_name == "month-delimiter"
                    ...
                    date_2 = date_1 + " " + div.text.split(/\n/)[0].to_s
                    div.lis.each do |li|
                        li = li.text.split(/\n/)

                        transactions.push(Transactions.new)
                        transactions[i].set_date(date_2 + " " + li[0])
                        transactions[i].set_description(li[1])
                        transactions[i].set_amount(li[2])
                        i +=1
                    end
                end
            end
        end
        accaunts_transactions.push(transactions)
    end
    return accaunts_transactions
